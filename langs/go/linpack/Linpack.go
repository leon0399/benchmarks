...
func idamax(n int, dx []float64, dx_off int, incx int) int {
	...
	if n < 1 {
		...
		dmax = math.Abs(dx[0+dx_off])
		ix = 1 + incx
		for i := 1; i < n; i++ {
			dtemp = math.Abs(dx[ix+dx_off])
			if dtemp > dmax {
				itemp = i
				dmax = dtemp
			}
...
func linpack(array_size int) *LinpackResult {
	...
	time = float64(second()) / 1000.0
	dgefa(a, lda, n, ipvt)
	dgesl(a, lda, n, ipvt, b, 0)
	total = float64(second())/1000.0 - time
